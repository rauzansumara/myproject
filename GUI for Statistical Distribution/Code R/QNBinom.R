QNBinom <- function(h,...){
  kua <- gwindow("Quantile Binomial Negatif")
  ...
  addHandlerChanged(ok,handler=function(h,...){
    ...
    if(avail[svalue(poin)]==1) { pelu <- qnbinom(svalue(x),svalue(coba),svalue(sukses))}
    if(avail[svalue(poin)]==2) {  pelu <- qnbinom(svalue(x),svalue(coba),svalue(sukses),lower.tail = F)}
    insert(Output,"")
