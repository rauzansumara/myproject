PNBinom <- function(h,...){
  pel <- gwindow("Peluang Binomial Negatif")
  ...
  addHandlerChanged(ok,handler=function(h,...){
    ...
    if(avail[svalue(poin)]==1) { pelu <- dnbinom(svalue(x),svalue(coba),svalue(sukses))}
    if(avail[svalue(poin)]==2) { pelu <- pnbinom(svalue(x),svalue(coba),svalue(sukses))}
    if(avail[svalue(poin)]==3) { pelu <- pnbinom(svalue(x),svalue(coba),svalue(sukses),lower.tail = F)}
    insert(Output,"")
