PChi <- function(h,...){
  pel <- gwindow("Peluang Chi-Square")
  ...
  addHandlerChanged(ok,handler=function(h,...){
    ...
    if(avail[svalue(poin)]==1) { pelu <- pchisq(svalue(x),svalue(df1))}
    if(avail[svalue(poin)]==2) { pelu <- pchisq(svalue(x),svalue(df1),lower.tail = F)}
    insert(Output,"")
