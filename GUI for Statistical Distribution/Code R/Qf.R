Qf <- function(h,...){
  ...
  addHandlerChanged(ok,handler=function(h,...){
    ...
    if(avail[svalue(poin)]==1) { pelu <- qf(svalue(x),svalue(df1),svalue(df2))}
    if(avail[svalue(poin)]==2) {  pelu <- qf(svalue(x),svalue(df1),svalue(df2),lower.tail = F)}
    insert(Output,"")
