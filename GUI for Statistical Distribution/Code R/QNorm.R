QNorm <- function(h,...){
  ...
  addHandlerChanged(ok,handler=function(h,...){
    ...
    if(avail[svalue(poin)]==1) { pelu <- qnorm(svalue(x),svalue(rata),svalue(SD))}
    if(avail[svalue(poin)]==2) {  pelu <- qnorm(svalue(x),svalue(rata),svalue(SD),lower.tail = F)}
    insert(Output,"")
