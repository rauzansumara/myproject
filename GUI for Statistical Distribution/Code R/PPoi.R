PPoi <- function(h,...){
  ...
  addHandlerChanged(ok,handler=function(h,...){
    ...
    if(avail[svalue(poin)]==1) { pelu <- dpois(svalue(x),svalue(lam))}
    if(avail[svalue(poin)]==2) { pelu <- ppois(svalue(x),svalue(lam))}
    if(avail[svalue(poin)]==3) { pelu <- ppois(svalue(x),svalue(lam),lower.tail = F)}
    insert(Output,"")
