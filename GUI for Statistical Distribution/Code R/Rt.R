Rt <- function(h,...){
  kua <- gwindow("Bangkitan t-Students")
  ...
  addHandlerChanged(ok,handler=function(h,...){
    pelu <- rt(svalue(nilai),svalue(df1))
    insert(Output,"")
    ...
    plotDistr(xv,yv,cdf = F,xlab="X",ylab="Densitas",
              main=paste('Distribusi t-Students : n =',svalue(nilai),'df =',svalue(df1)))
    ...
    if(avail[svalue(poin)]==2) { 
      ...
      p <- round(pt(x,svalue(df1),lower.tail = F),3)
      xx <- seq(x,maxx,0.01)
      yy <- dt(xx,svalue(df1))
      cord.x <- c(x,xx,maxx) 
      cord.y <- c(0,yy,0) 
